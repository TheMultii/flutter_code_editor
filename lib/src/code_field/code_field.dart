...
class CodeField extends StatefulWidget {
  ...
  /// Whether to separate gutter from editor with a small padding
  final bool separateGutterFromEditor;
  ...
  const CodeField({
    ...
    this.focusNode,
    this.onChanged,
    this.separateGutterFromEditor = false,
...
class _CodeFieldState extends State<CodeField> {
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            if (widget.separateGutterFromEditor) const SizedBox(width: 8),
  ...
  double _getCaretHeight(TextPainter textPainter) {
    ...
    return (widget.controller.selection.base.offset > 0)
        ? caretFullHeight ?? 0
        : 0;
  }

  double _getPopupLeftOffset(TextPainter textPainter) {
    final gStyle = widget.gutterStyle;

    return _getCaretOffset(textPainter).dx +
        (gStyle.showErrors &&
                gStyle.showFoldingHandles &&
                gStyle.showLineNumbers
            ? 88
            : 0);
    // return max(
  ...
  int _getLineFromCaret() {
    final cursorPosition = widget.controller.selection.baseOffset;
    final text = widget.controller.text;

    if (cursorPosition == 0) {
      return 1; // Cursor is on the first line
    }
  ...
  double _getPopupTopOffset(TextPainter textPainter, double caretHeight) {
    return max(
      _getCaretOffset(textPainter).dy +
          ...
          widget.padding.top -
          _codeScroll!.offset +
          ((_editorOffset?.dy != null ? _editorOffset!.dy + 48 : 0) * .913),
      0,
    );
  }
